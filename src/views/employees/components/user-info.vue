...
import { getUserDetailById } from "@/api/user"; // 员工公司身份详情接口
import {
  getPersonalDetail,
  saveUserDetailById,
  updatePersonal,
} from "@/api/employees"; // 员工个人身份详情接口 保存信息接口
...
export default {
  ...
  methods: {
    async getUserDetailById() {
      // 在data中userId已被赋值，找到上面定义的属性名并将结果赋给
    ...
    async saveUser() {
      ...
      if (fileList.some((item) => !item.upload)) {
        // 说明此时图片还没上传成功 upload为true是上传成功的标志
        this.$message.warning("此时图片没有上传完成");
        return;
      }
      // 此时userInfo中的staffPhoto还没有更新 需合并
      // await saveUserDetailById(this.userInfo);
      // 如果fileList中有对象则将第一个的URL地址赋给staffPhoto
